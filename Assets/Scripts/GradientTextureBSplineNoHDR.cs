...
namespace Packages.GradientTextureGenerator.Runtime
{
    ...
    /// </summary>
    [CreateAssetMenu(fileName = "NewGradientName", menuName = "Texture/Gradient-BSplineNoHDR")]
    public class GradientTextureBSplineNoHDR : ScriptableObject, IEquatable<Texture2D>, ISerializationCallbackReceiver,
        IGradientTextureForEditor
    {
        ...
        [SerializeField] AnimationCurve _sampleCurve = AnimationCurve.Linear(0, 0, 1, 1);
        ...
        public static implicit operator Texture2D(GradientTextureBSplineNoHDR asset) => asset.GetTexture();
        ...
        public void FillColors(bool useRGB)
        {
            ...
            for (int y = 0; y < _height; y++)
            {
                ...
                for (int x = 0; x < _width; x++)
                {
                    ...
                    Color color = Color.Lerp(_horizontalBottom.Evaluate(_sampleCurve.Evaluate(tHorizontal)),
                        _horizontalTop.Evaluate(_sampleCurve.Evaluate(tHorizontal)),
                        _sampleCurve.Evaluate(tVertical));
